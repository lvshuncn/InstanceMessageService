...
export class User {
  ...
  static fromMongo(doc) {
  	var user = new User(doc._id, doc.name, doc.account, doc.passsword, doc.avatar);
  	return user;
  }
...
export class UserDAO {
  ...
  static * insert(user) {
  	var result = yield this._users.$innsert(User.toMongo(user));
  	return result[0]._id;
  }
  ...
  static * updateProperty(id, property) {
  	var update = {};
  	if (property.nickName) update.name = property.name;
